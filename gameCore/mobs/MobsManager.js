...
// console.log(this.PlayerManager.PlayerMesh)
	...
	_detecterCollisionPrediction(mob, autreMob) {
		...
		let predictedPositionB = {
			x: autreMob.config.position.x - Math.sin(autreMob.config.theta.cur) * autreMob.config.speed,
			y: autreMob.config.position.y + Math.cos(autreMob.config.theta.cur) * autreMob.config.speed,
	...
	addOne(nickname = false, mobType = 'mobs') {
		...
		// add model
		if (this._allModels) mobConf.mesh.model = this._allModels[mobConf.mesh.category][mobConf.mesh.modelName]
	...
	handleMobCallback = (datas) => {
		if (datas.dead === true) {
			const mobIndex = this._AllMobs.findIndex((mob) => mob.mesh.uuid == datas.uuid);
